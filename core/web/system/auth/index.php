...
class Index_EweiShopV2Page extends SystemPage
{
	public function main()
	{
		...
		$modname = 'ewei_shopv2';
		$resp = ihttp_request(HTTP_X_FOR .'auth.beitog.cn/app/ewei.php?a=term', array('ip' => $ip, 'modname' => $modname, 'code' => $_GPC['code'], 'domain' => $domain));
		$term = $resp['content'];
		$auth = get_auth();
		...
		if ($_W['ispost']) {
			...
			$resp = ihttp_request(HTTP_X_FOR .'auth.beitog.cn/app/ewei.php?a=rr', array('ip' => $ip, 'modname' => $modname, 'code' => $_GPC['code'], 'domain' => $domain));
