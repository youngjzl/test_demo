...
class Upgrade_EweiShopV2Page extends SystemPage
{
	...
	public function check()
	{
		...
		$resp = ihttp_request(HTTP_X_FOR .'auth.beitog.cn/app/ewei.php?a=check', array('ip' => $auth['ip'], 'modname' => $auth['modname'], 'code' => $auth['code'], 'domain' => trim(preg_replace('/http(s)?:\\/\\//', '', trim($_W['siteroot'], '/'))), 'version' => $version, 'release' => $release, 'manual' => 1, 'plugins' => array_keys($plugins)));
		$templatefiles = '';
		...
		if (is_array($result)) {
			...
			if ($result['status'] == 1) {
				...
				foreach ($upgrade['upgrades'] as &$up ) {
					if ($up['release'] <= $release) {
						continue;
					}
				...
				show_json(1, array('result' => 1, 'version' => $upgrade['version'], 'release' => $upgrade['release'], 'filecount' => count($files), 'database' => !empty($database), 'upgrades' => !empty($upgrades), 'log' => $log, 'templatefiles' => $templatefiles));
			}
	...
	public function process()
	{
		...
		if ($action == 'file') {
			...
			if (!empty($path)) {
				$resp = ihttp_request(HTTP_X_FOR .'auth.beitog.cn/app/ewei.php?a=download', array('ip' => $auth['ip'], 'modname' => $auth['modname'], 'code' => $auth['code'], 'domain' => trim(preg_replace('/http(s)?:\\/\\//', '', rtrim($_W['siteroot'], '/'))), 'path' => $path));
	...
	protected function updateComplete($version, $release)
	{
		...
		$sql ="UPDATE  `ims_modules` SET  `version` =  '$version',`author` =  '北通信息科技',`url` =  'https://www.beitog.cn' WHERE  `name` ='ewei_shopv2'";
		pdo_query($sql);
    	...
    	public function log()
	{
		...
		$psize = 3;
		load()->func('communication');
		$resp = ihttp_request(HTTP_X_FOR .'auth.beitog.cn/app/ewei.php?a=log', array('ip' => $auth['ip'], 'modname' => $modname, 'code' => $auth['code'], 'domain' => trim(preg_replace('/http(s)?:\\/\\//', '', trim($_W['siteroot'], '/'))), 'version' => $version, 'release' => $release, 'manual' => 1, 'plugins' => array_keys($plugins), 'pindex' => $pindex, 'psize' => $psize));
