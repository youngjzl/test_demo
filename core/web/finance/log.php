...
class Log_EweiShopV2Page extends WebPage 
{
	protected function main($type = 0) 
	{
		...
		$sql = "select log.id,log.openid,log.logno,log.type,log.status,log.rechargetype,log.sendmoney,log.money,log.createtime,log.realmoney,log.deductionmoney,log.charge,log.remark,log.alipay,log.bankname,log.bankcard,log.realname as applyrealname,log.applytype,log.operator,log.operator_ip,m.nickname,m.id as mid,m.avatar,m.level,m.groupid,m.realname,m.mobile,g.groupname,l.levelname from " . tablename("ewei_shop_member_log") . " log " . " left join " . tablename("ewei_shop_member") . " m on m.openid = log.openid " . " left join " . tablename("ewei_shop_member_group") . " g on g.id = m.groupid " . " left join " . tablename("ewei_shop_member_level") . " l on l.id = m.level " . " where 1 " . $condition . " " . $condition1 . " GROUP BY log.id ORDER BY log.createtime DESC ";
		...
		if( !empty($list) ) 
		{
			...
			{
			    //2019108
                $list[$key]["operator_ip"]=!empty($value['operator_ip']) ?  $value['operator_ip'] : '无记录';
			    $operator=pdo_get('users', array('uid' => $value['operator']), array('username'));
			    $list[$key]["operator"]= !empty($operator['username']) ?  $operator['username'] : '无记录';
				//--end
