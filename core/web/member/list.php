...
class List_EweiShopV2Page extends WebPage 
{
	public function main() 
	{
		...
		foreach( $list as &$row ) 
		{
            ...
            //最近历史消费记录
            $consumption=pdo_fetchall("select id,createtime,price from " . tablename("ewei_shop_order") . " where uniacid=:uniacid and openid=:openid and status=3 order by createtime desc", array( ":uniacid" => $_W["uniacid"], ":openid" => $row["openid"]));
            $row["consumption"]['shopp']=$consumption;
            $row["credit1"] = m("member")->getCredit($row["openid"], "credit1");
			$row["credit2"] = m("member")->getCredit($row["openid"], "credit2");
		}

        $recently_time_price='';//上次消费金额
        $on_time_price='';//上上次消费金额
        //上上次一天内购买商品所属分类（全部）
        $cates=[];
        $temp2=[];//存入已排序好的值
        $category = m('shop')->getFullCategory();//分类
        $num=0;//最大为三只去三个分类
        //对上上次一天内订单进行排序按照分类id出现次数从多-》少取前三
        foreach ($list as $k=>$lists_cates){
            //金额&&时间
            if (!empty($lists_cates["consumption"]['shopp'])){
                //最近一次消费时间
                $timed=strtotime(date('Y-m-d',$lists_cates['consumption']['shopp'][0]['createtime']));
                $list[$k]['consumption']['recently_time']=$timed;
                //此时间的第二天
                $timed_2=strtotime('+1 day',$timed);
                //价格和金额
                $list2=pdo_fetch("select createtime from " . tablename("ewei_shop_order") . " where uniacid=:uniacid and openid=:openid and status=3 and createtime<:timed order by createtime desc LIMIT 1", array( ":uniacid" => $_W["uniacid"], ":openid" => $lists_cates["openid"],':timed'=>$timed_2));
                for ($i=0; $i<count($lists_cates["consumption"]['shopp']);$i++){
                    if ($lists_cates['consumption']['shopp'][$i]['createtime']>=$timed && $lists_cates['consumption']['shopp'][$i]['createtime']<$timed_2){
                        $recently_time_price+=floatval($lists_cates['consumption']['shopp'][$i]['price']);
                    }
                    if (!empty($list2)){
                         //上上次消费时间
                         $time2=strtotime(date('Y-m-d',$list2['createtime']));
                         $list[$k]['consumption']['on_time']=$time2;
                         //此时间的第二天
                         $timed2_2=strtotime('+1 day',$time2);
                         //上上次一天内金额
                         if ($lists_cates['consumption']['shopp'][$i]['createtime']>=$time2 && $lists_cates['consumption']['shopp'][$i]['createtime']<$timed2_2){
                             $on_time_price+=floatval($lists_cates['consumption']['shopp'][$i]['price']);
                         }
                         //计算相差天数
                         $diff= $timed - $time2 ;
                         $days =abs(round($diff / 86400));
                         $list[$k]['consumption']['days']=$days;//相差天数

                         $list3=pdo_fetch("select createtime from " . tablename("ewei_shop_order") . " where uniacid=:uniacid and openid=:openid and status=3 and createtime<:timed2 order by createtime desc", array( ":uniacid" => $_W["uniacid"], ":openid" => $lists_cates["openid"],':timed2'=>$timed2_2));
                     }
                }
                //上上上次购买时间
                if (!empty($list3)){
                    //上上上次购买的时间
                    $time3=strtotime('+1 day',strtotime(date('Y-m-d',$list3['createtime'])));
                }
                $list[$k]['consumption']['recently_time_price']=$recently_time_price;
                $list[$k]['consumption']['on_time_price']=$on_time_price;
                $recently_time_price='';//清空
                $on_time_price='';
            //前一次最近时间
            if (!empty($timed_2)){
                $newtime=$timed_2;
                ...
                //最近三次三天购买的订单
                $list4=pdo_fetchall("select id from " . tablename("ewei_shop_order") . " where uniacid=:uniacid and openid=:openid and status=3 and createtime<:newtime  order by createtime desc", array( ":uniacid" => $_W["uniacid"], ":openid" => $lists_cates["openid"],':newtime'=>$newtime));
                foreach ($list4 as $orderid){
                    $lists_cates['consumption']["goodsorder_cates"][]= pdo_fetchall("select g.cates from " . tablename("ewei_shop_order_goods") . " og " . " left join " . tablename("ewei_shop_goods") . " g on g.id=og.goodsid " . " left join " . tablename("ewei_shop_goods_option") . " op on og.optionid = op.id " . " where og.uniacid=:uniacid and og.orderid=:orderid", array(":uniacid" =>$_W["uniacid"], ":orderid" => $orderid['id']));
                }
            }
            //排序分类
            if (!empty($lists_cates['consumption']["goodsorder_cates"])){

                foreach ($lists_cates['consumption']["goodsorder_cates"] as $lists_catess){
                    if (count($lists_catess) == count($lists_catess, 1)) {
                        $cates[]=$lists_catess;
                    } else {
                        foreach ($lists_catess as $lists_catesss){
                            $cates[]=$lists_catesss;
                        }
                    }
                }
                foreach ($cates as $k3=>$catess){
                    $temp[$k3]=join(',',$catess);
                }
                foreach ($temp as $cates_1s){
                    foreach (explode(",",$cates_1s) as $cates_1ss){
                        array_push($temp2,$cates_1ss);
                    }
                }
            }
                //取值
                if (!empty($temp2)) {
                    $cate_fenlei = array_count_values($temp2);
                    foreach ($category as $categorys) {
                        foreach ($cate_fenlei as $k4=>$cate_fenlei_id){
                            if ($categorys['id'] == $k4) {
                                $lists_cates['consumption']['cates'][]=$categorys['name'];
                            }
                        }
                    }
                    if (!empty($lists_cates['consumption']['cates'])){
                        foreach ($lists_cates['consumption']['cates'] as $k_cat=>$catess){
                            if ($k_cat<3){
                                $list[$k]["consumption"]['cates'][]=$catess;
                            }else{
                                continue;
                            }
                        }
            ...
            unset($list[$k]["consumption"]['shopp']);
        }
        unset($lists_cates);
        unset($row);
        if( $_GPC["export"] == "1" )
