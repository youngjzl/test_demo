    ...
    //修复操作员操作反映慢
        if (empty(self::$allPerms)) {
            $perms = array(
                ...
                "apiuser" => $this->perm_apiuser()
            );
            self::$allPerms = $perms;
        }
        return self::$allPerms;
    }
    protected function perm_apiuser() {
        return ($this->isopen("apiuser") && $this->is_perm_plugin("apiuser") ? array(
            "text" => m("plugin")->getName("apiuser") ,
            "main" => "查看授权列表",
            "add" => "添加授权用户"
        ) : array());
    }
