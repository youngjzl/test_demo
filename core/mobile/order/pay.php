...
class Pay_EweiShopV2Page extends MobileLoginPage 
{
	public function main() 
	{
		...
		//生成海关支付记录
		$haiguan_data = pdo_fetch("select * from " . tablename("haiguan_data") . " where `orderNo`=:orderNo limit 1", array( ":orderNo" => $order["ordersn"] ));
		if(empty($haiguan_data)){
			$haiguan = array(
				"orderNo" => $order["ordersn"],
				"create_time" => time()
			);
			pdo_insert("haiguan_data", $haiguan);
		}
		...
		//添加合利宝支付
		$helipay = array( "success" => false );
		if( isset($set["pay"]) && $set["pay"]["helipay"] == 1 && is_weixin()) 
		{
			$helipay = array("success" => true);
		}
		...
		$payinfo = array( "orderid" => $orderid, "ordersn" => trim($log["tid"]), "helipay" => $helipay, "credit" => $credit, "alipay" => $alipay, "wechat" => $wechat, "cash" => $cash, "money" => $order["price"] );
		if( is_h5app() ) 
		{
        ...
        //根据ispay_yue判断是否用余额支付
        $goods_shopid=json_decode($order['ceyhshopid'],true);
        $goods_shopid=implode(',',$goods_shopid);
        $goods = pdo_fetchall("select goodsbusinesstype from " . tablename("ewei_shop_goods") . " where id in($goods_shopid) and uniacid=:uniacid ", array(":uniacid" => $uniacid ));
        foreach ($goods as $ispay_yue){
            $ispay_yues[]=$ispay_yue['goodsbusinesstype'];
        }
        $is_yue=array_search('2',$ispay_yues);//----end
	...
	public function complete()
	{
		...
		$type = $_GPC["type"];
		if( !in_array($type, array( "wechat", "helipay", "alipay", "credit", "cash" )) ) 
		{
			if( $_W["ispost"] ) 
			{
				show_json(0, "未找到支付方式");
	...
	public function hsuccess() 
	{
		...
		m("order")->setOrderPayType($orderid, 24, $order["ordersn"]);
		$ret = array( );
		$ret["result"] = "success";
		$ret["type"] = "weich";
		$ret["from"] = "return";
		...
		$ret["uniacid"] = $_W["uniacid"];
		$pay_result = m("order")->payResult($ret);
        pdo_update("ewei_shop_order", array( "cashtime" => time(), "status" => "1"), array( "id" => $order["id"] ));
		m("notice")->sendOrderMessage($order["id"]);
		@session_start();
		$_SESSION[EWEI_SHOPV2_PREFIX . "_order_pay_complete"] = 1;
		if( $_W["ispost"] ) 
		...
		{
			header("location:" . mobileUrl("order/pay/success", array( "id" => $orderid, "result" => $pay_result )));
		}
		exit();
	}
