...
class Cart_EweiShopV2Page extends MobileLoginPage 
{
	public function main() 
	{
		...
		if( $merch_plugin && $merch_data["is_openmerch"]&&!empty($merchid))
		{
		    //跟随首页的底部菜单改变购物车的菜单
	...
	public function get_list() 
	{
		...
		{
		    //2019-9-11/$where_merchid多商户商城商品判断
		    $where_merchid=!empty($_GPC['merchid']) || isset($_GPC['merchid']) ? 'and g.merchid!=0' : 'and g.merchid=0';
			$condition = " and f.uniacid= :uniacid and f.openid=:openid and f.deleted=0 ".$where_merchid;
		}
		...
		foreach( $list as &$g ) 
		{
			...
			if($g['goodsbusinesstype']==4){
				$g["rateprice"] = $g["productprice"] * $g["rate5"];
			}
			if( empty($g["selected"]) ) 
			{
				$ischeckall = false;
		...
		else 
		{
          ...
          $ceyhurl=json_decode($ceyhurl_json,true)['ceyhurl'];
            ...
            if (!empty($ceyhurl) && !empty($item)){
                ...
                $ceyhcha=$item['enoughmoney']>=$ceyhprice ? $item['enoughmoney']-$ceyhprice : 0;
                $ceyhitem[]=$item;
                //--end
            }
		}
	...
	public function add()
	{
		...
		{
			$rateprice=0;
			if($goods['goodsbusinesstype']==3){
				$rateprice = ($goods["marketprice"] + $goods['dispatchprice']) * $goods["rate5"];
			}
			$data = array( "rateprice"=>$rateprice,"uniacid" => $_W["uniacid"], "merchid" => $goods["merchid"], "openid" => $_W["openid"], "goodsid" => $id, "optionid" => $optionid, "marketprice" => $goods["marketprice"], "total" => $total, "selected" => 1, "diyformid" => $diyformid, "diyformdata" => $diyformdata, "diyformfields" => $diyformfields, "createtime" => time() );
	...
	public function submit()
	{
		...
		$params = array( ":uniacid" => $uniacid, ":openid" => $openid );
		$sql = "SELECT f.id,f.total,f.goodsid,g.total as stock, o.stock as optionstock, g.maxbuy,g.title,g.thumb,ifnull(o.marketprice, g.marketprice) as marketprice," . " g.productprice,o.title as optiontitle,f.optionid,o.specs,g.minbuy,g.maxbuy,g.usermaxbuy,g.unit,g.merchid,g.checked,g.isdiscount_discounts,g.isdiscount,g.isdiscount_time,g.isnodiscount,g.discounts,g.merchsale" . " ,f.selected,g.status,g.deleted as goodsdeleted,g.type,g.intervalfloor,g.intervalprice,g.goodsbusinesstype  FROM " . tablename("ewei_shop_member_cart") . " f " . " left join " . tablename("ewei_shop_goods") . " g on f.goodsid = g.id " . " left join " . tablename("ewei_shop_goods_option") . " o on f.optionid = o.id " . " where 1 " . $condition . " ORDER BY `id` DESC ";
		$list = pdo_fetchall($sql, $params);
		if( empty($list) )
		{
        ...
        if(count($list)>1){
            foreach ($list as $key =>$value){
                if($value['goodsbusinesstype']==2){
                    $admin[]=array('admin'=>$value['goodsbusinesstype']);
                }
            }
            if (!empty($admin)){
                $adminCount = count($admin);
                if(count($list)!=$adminCount){
                    show_json(0, "保税和非保税商品无法同时支付");
                }
            }
