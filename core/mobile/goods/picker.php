...
class Picker_EweiShopV2Page extends MobilePage 
{
	public function main() 
	{
		...
		if ($goods['isdiscount'] && (time() <= $goods['isdiscount_time_end'] && time() >= $goods['isdiscount_time']))
		{
			$isdiscount = true;
...
//								    $options[$k]['seecommission'] = floatval(str_replace('%', '', $value[0]) / 100) * $price;
                                    $options[$k]['seecommission'] = floatval(str_replace('%', '', $value[0]) / 100) * $v['marketprice'];//修改2019-8-8改用每个单独的价格
									continue;
								}
