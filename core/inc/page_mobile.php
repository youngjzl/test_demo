...
class MobilePage extends Page
{
    ...
    public function footerMenus($diymenuid = NULL, $ismerch = false, $texts = array(  ))
    {
        ...
        if( !empty($diymenuid) ) 
        {
            ...
            if(!empty($merchid))
            {
                //跟随首页的底部菜单改变个人中心订单页面的菜单
                $diypagedata = p("merch")->getSet("diypage",intval($merchid));
                $where = " where uniacid=:uniacid and merch=:merchid and id=:id";
                $params = array(":uniacid" => $_W["uniacid"],":merchid"=>intval($merchid),':id'=>$diypagedata['page']['home']);
                $page = pdo_fetch("select * from " . tablename("ewei_shop_diypage") . $where . " limit 1 ", $params);
                $diymenuid=$page['diymenu'];
            }
            ...
            if( !empty($diypageid) ) 
            {
                ...
                if( !empty($page) )
                {
                    ...
                    if( $type == "home" ) 
                    {
                        ...
                        {
                            //2019923多增加了一个字段is_opdd用于判断订单提醒状态
                            $order = pdo_fetch("select id,price,is_opdd  from " . tablename("ewei_shop_order") . " where uniacid=:uniacid and status = 0 and paytype<>3 and openid=:openid order by createtime desc limit 1", array( ":uniacid" => $_W["uniacid"], ":openid" => $_W["openid"] ));
                            if( !empty($order) )
                            {
