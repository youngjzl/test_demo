...
class Detail_EweiShopV2Page extends MobilePage
{
    public function main()
    {
        ...
        //记录用户点击了商品几次20191126
        if (!empty($id)){
            m('cnxhdata')->setclickgoodsnum($id);
        }//--记录用户点击了几次end
        ...
        //税费计算
        $goods["rateprice"] = 0;
        if(intval($goods['goodsbusinesstype']) == 3){
            $goods["rateprice"] = (floatval($goods["marketprice"]) + floatval($goods['dispatchprice'])) * (floatval($goods['rate5']) / 100);
        }
        if(intval($goods['goodsbusinesstype']) == 5){
            $goods["rateprice"] = floatval($goods["marketprice"]) * (floatval($goods['rate4']) / 100);
        }
        ...
        //促销时间判断
        if( $goods["isdiscount"] && time() >= $goods["isdiscount_time"] && time() <= $goods["isdiscount_time_end"])
        {
        ...
        //判断是否是新会员的价格
        if ($minprice==$maxprice && $goods['new_user_price']>0){
            ...
            if (!empty($info['openid'])){
                ...
                if (empty($user_orders) && $goods['new_user_price']>0){
                    $user_orders_status=1;
                    $minprice=$goods['new_user_price'];
                    $maxprice=$minprice;
                }
            }
        }
        unset($user_orders);//=====end
    ...
    public function get_code()
    {
        ...
        $goods = pdo_fetch("select id,minprice,minprice,maxprice,thumb_url,thumb,title,isdiscount_time,isdiscount_time_end,isdiscount_discounts from " . tablename("ewei_shop_goods") . " where id=:id and uniacid=:uniacid limit 1", array( ":id" => $id, ":uniacid" => $_W["uniacid"] ));
        $goods['isdiscount_discounts']=json_decode($goods['isdiscount_discounts'],true);
        $member = m("member")->getMember($openid);
        $commission_data = m("common")->getPluginset("commission");
        $goodscode = "";
        $parameter = array( );
        if( com("goodscode") )
        {
            if( $goods["minprice"] == $goods["maxprice"] )
            {
                ...
                if (time()>$goods["isdiscount_time"] && time()<$goods["isdiscount_time_end"]){
                    $price="¥" . $goods["isdiscount_discounts"]['default']['option0'];
                }
            ...
            else
            {
                ...
                if (time()>$goods["isdiscount_time"] && time()<$goods["isdiscount_time_end"]){
                    $price = "¥" . $goods["isdiscount_discounts"]['default']['option0'] . " ~ " . $goods["maxprice"];
                }
            }
