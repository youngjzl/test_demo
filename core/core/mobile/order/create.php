...
class Create_EweiShopV2Page extends MobileLoginPage 
{
	...
	public function main() 
	{
		...
		$member = m("member")->getMember($_W["openid"]);//用户信息
		$show_card = true;
		if( p("exchange") ) 
		{
			...
			if( !empty($exchangeOrder) ) 
			{
				...
				//$rate_price = $_SESSION["rate_price"];
				if( $_GPC["dflag"] == "1" ) 
				{
					$exchangeprice = 0;
				}
				$exchangerealprice = $exchangeprice + $exchangepostage;// + $rate_price;
				$show_card = false;
			}
			else 
			{
		...
		if( !$packageid ) 
		{
			...
			$member["carrier_mobile"] = (empty($member["carrier_mobile"]) ? $member["mobile"] : $member["carrier_mobile"]);//电话号码（账号）
			$share = m("common")->getSysset("share");
//			echo "<pre>";var_dump($share);die;
			...
			if( empty($id) ) 
			{
				...
				else 
				{
					if( empty($exchangeOrder) ) 
					{
						$sql = "SELECT c.goodsid,c.total,g.goodsbusinesstype,g.maxbuy,g.type,g.intervalfloor,g.intervalprice,g.issendfree,g.isnodiscount,g.ispresell,g.presellprice as gpprice,o.presellprice,g.preselltimeend,g.presellsendstatrttime,g.presellsendtime,g.presellsendtype" . ",g.weight,o.weight as optionweight,g.title,g.thumb,ifnull(o.marketprice, g.marketprice) as marketprice,o.title as optiontitle,c.optionid," . " g.storeids,g.isverify,g.isforceverifystore,g.deduct,g.manydeduct,g.virtual,o.virtual as optionvirtual,discounts," . " g.deduct2,g.ednum,g.edmoney,g.edareas,g.diyformtype,g.diyformid,diymode,g.dispatchtype,g.dispatchid,g.dispatchprice,g.minbuy " . " ,g.isdiscount,g.isdiscount_time,g.isdiscount_time_end,g.isdiscount_discounts,g.cates,g.new_user_price, " . " g.virtualsend,invoice,o.specs,g.merchid,g.checked,g.merchsale," . " g.buyagain,g.buyagain_islong,g.buyagain_condition, g.buyagain_sale, g.hasoption" . " FROM " . tablename("ewei_shop_member_cart") . " c " . " left join " . tablename("ewei_shop_goods") . " g on c.goodsid = g.id " . " left join " . tablename("ewei_shop_goods_option") . " o on c.optionid = o.id " . " where c.openid=:openid and c.selected=1 and  c.deleted=0 and c.uniacid=:uniacid  order by c.id desc";
						$goods = pdo_fetchall($sql, array( ":uniacid" => $uniacid, ":openid" => $openid ));
					}
					else 
					{
						if( p("exchange") ) 
						{
							$sql = "SELECT c.goodsbusinesstype,c.goodsid,c.total,g.maxbuy,g.type,g.intervalfloor,g.intervalprice,g.issendfree,g.isnodiscount,g.ispresell,g.presellprice as gpprice,o.presellprice,g.preselltimeend,g.presellsendstatrttime,g.presellsendtime,g.presellsendtype" . ",g.weight,o.weight as optionweight,g.title,g.thumb,ifnull(o.marketprice, g.marketprice) as marketprice,o.title as optiontitle,c.optionid," . " g.storeids,g.isverify,g.isforceverifystore,g.deduct,g.manydeduct,g.virtual,o.virtual as optionvirtual,discounts," . " g.deduct2,g.ednum,g.edmoney,g.edareas,g.edareas_code,g.diyformtype,g.diyformid,diymode,g.dispatchtype,g.dispatchid,g.dispatchprice,g.minbuy " . " ,g.isdiscount,g.isdiscount_time,g.isdiscount_time_end,g.isdiscount_discounts,g.cates,g.isfullback,g.new_user_price, " . " g.virtualsend,invoice,o.specs,g.merchid,g.checked,g.merchsale,g.unite_total," . " g.buyagain,g.buyagain_islong,g.buyagain_condition, g.buyagain_sale, g.hasoption" . " FROM " . tablename("ewei_shop_exchange_cart") . " c " . " left join " . tablename("ewei_shop_goods") . " g on c.goodsid = g.id " . " left join " . tablename("ewei_shop_goods_option") . " o on c.optionid = o.id " . " where c.openid=:openid and c.selected=1 and  c.deleted=0 and c.uniacid=:uniacid  order by c.id desc";
							$goods = pdo_fetchall($sql, array( ":uniacid" => $uniacid, ":openid" => $openid ));
						}
					}
				}
			...
			else 
			{
				...
				else 
				{
					...
					//2019-8-7增加一个促销结束时间isdiscount_time_end
					$sql = "SELECT id as goodsid,type,title,weight,issendfree,isnodiscount,ispresell,presellprice,goodsbusinesstype,rate5," . " thumb,marketprice,liveprice,islive,storeids,isverify,isforceverifystore,deduct,hasoption,preselltimeend,presellsendstatrttime,presellsendtime,presellsendtype," . " manydeduct,`virtual`,maxbuy,usermaxbuy,discounts,total as stock,deduct2,showlevels," . " ednum,edmoney,edareas,edareas_code,unite_total,diyfields," . " diyformtype,diyformid,diymode,dispatchtype,dispatchid,dispatchprice,cates,minbuy, " . " isdiscount,isdiscount_time,isdiscount_time_end,isdiscount_discounts,isfullback, " . " virtualsend,invoice,needfollow,followtip,followurl,merchid,checked,merchsale, " . " buyagain,buyagain_islong,buyagain_condition, buyagain_sale,bargain,new_user_price" . $threensql . $ishotelsql . " FROM " . tablename("ewei_shop_goods") . " where id=:id and uniacid=:uniacid  limit 1";
					$data = pdo_fetch($sql, array( ":uniacid" => $uniacid, ":id" => $id ));
            ...
            //购物车根据goodsbusinesstypeid判断身份证
            $iscarids=array();
            foreach($goods as $goodsbusinesstypes){
                if ($goodsbusinesstypes['goodsbusinesstype']=='2'|| $goodsbusinesstypes['goodsbusinesstype']=='3'|| $goodsbusinesstypes['goodsbusinesstype']=='6'){
                    $iscarids[] = $goodsbusinesstypes['goodsbusinesstype'];
                }
            }
            rsort( $iscarids);
            $goods[0]['goodsbusinesstypeid']=$iscarids[0];//赋值给第一条商品的goodsbusinesstypeid来判断是否需要身份证
            //----- end 购物车根据goodsbusinesstype判断身份证
			...
			foreach( $goods as &$g ) 
			{
				...
				if( $taskcut || $g["seckillinfo"] && $g["seckillinfo"]["status"] == 0 ) 
				{
                    ...
                    if (empty($user_orders) && $g['new_user_price']>0){
                        $g["ggprice"] = $g["new_user_price"] * $g["total"];
                        $new_user_price1+=($g["marketprice"] - $g["new_user_price"])* $g["total"];
                    }
				...
				if($g["goodsbusinesstype"] == 3){//判断是否是海外商品
					$rateprice = round(floatval($g["ggprice"]) / (100 + floatval($g["rate5"])) * 100, 2);
					$rate_price += (floatval($g["ggprice"]) - $rateprice);
					$goodsprice1 += floatval($rateprice);
				}else{//非海外商品
					$goodsprice1 += floatval($realprice);
				}
              	$g["rateprice"] = $rate_price;
			}
			unset($g);
			if( $isverify ) 
			{
        ...
        //20191022
        $ceyhprice=0.00;
        $ceyhurl_json = file_get_contents(EWEI_SHOPV2_CORE.'/web/sale/ceyh/ceyhurl.json');
        $ceyhurl=json_decode($ceyhurl_json,true)['ceyhurl'];
        $item=pdo_fetch('SELECT enoughmoney,enoughdeduct,status,shopid FROM ' . tablename('ewei_shop_ceyh'). (' where status=1 order by enoughmoney asc'),$params);
        if (!empty($ceyhurl) && !empty($item)){
            //20191022凑额优惠
            $ceyhshopeid=explode(',',$item['shopid']);
            foreach ($goods as $lists){
                foreach ($ceyhshopeid as $id){
                    if ($lists['goodsid']==$id){
                        $ceyhprice+=$lists['marketprice']*$lists['total'];
                    }
                }
            }
            if ($ceyhprice>=$item['enoughmoney']){
                $realprice= bcsub($realprice,floatval($item['enoughdeduct']),2);
            }//--end
        }
		...
		$memberInfo = pdo_get('ewei_shop_member', array('openid' => $_W['openid'], 'uniacid' => $_W['uniacid']), array('idnumber', 'idcard_front', 'idcard_back'));
	...
	public function getcouponprice() 
	{
		...
		$goodsarr = $_GPC["goods"];//购买的商品
		$goodsprice = $_GPC["goodsprice"];//商品的价格
		$discountprice = $_GPC["discountprice"];//商品的折扣
		$isdiscountprice = $_GPC["isdiscountprice"];
		$contype = intval($_GPC["contype"]);//优惠券的类型
	...
	public function caculate() 
	{
		...
		if( is_array($goodsarr) ) 
		{
			...
			foreach( $goodsarr as &$g ) 
			{
				...
				//新会员价格判断
                if (empty($user_orders) && $data['new_user_price']>0){
                    $data["ggprice"] = $data["new_user_price"] * $data["total"];
                }//---新会员价格判断
       ...
       //20191022
        $ceyhurl_json = file_get_contents(EWEI_SHOPV2_CORE.'/web/sale/ceyh/ceyhurl.json');
        $ceyhurl=json_decode($ceyhurl_json,true)['ceyhurl'];
        $item=pdo_fetchall('SELECT enoughmoney,enoughdeduct,status FROM ' . tablename('ewei_shop_ceyh'). (' where status=1 order by enoughmoney asc'),$params);
        if (!empty($ceyhurl) && !empty($item)) {
            //20191022凑额优惠
            $ceyhshopeid=explode(',',$item['shopid']);
            foreach ($item as $lists){
                foreach ($ceyhshopeid as $id){
                    if ($lists['goodsid']==$id){
                        $ceyhprice+=$lists['marketprice'];
                    }
                }
            }

            if ($ceyhprice>=$item['enoughmoney']){
                $realprice=$realprice-$ceyhitem['enoughdeduct'];
                $deductcredit2=$deductcredit2-$ceyhitem['enoughdeduct'];
            }//--end
        }
	...
	public function submit() 
	{
		...
		if( $giftid ) 
		{
			...
			if( $giftdata["giftgoodsid"] ) 
			{
				...
				foreach( $giftgoodsid as $key => $value ) 
				{
					$giftitem = pdo_fetch("select id as goodsid,title,thumb,goodsbusinesstype,rate,rate1,rate2,rate3,rate4,rate5 from " . tablename("ewei_shop_goods") . " where uniacid = " . $uniacid . " and status = 2 and id = " . $value . " and deleted = 0 ");
					if( empty($giftitem) ) 
					{
						continue;
					}
		...
		foreach( $goods as $g ) 
		{
			...
			switch(intval($g["goodsbusinesstype"])){
				case 4:
					$goodstotal = $goodstotal + ($goodstotal * $rate5 / 100);
					break;
				case 5:
					$goodstotal = $goodstotal + ($goodstotal * $rate4 / 100);
					break;
			}
		...
		foreach( $goods as $g )
		{
            ...
            //是否是新会员的价格
            if (empty($user_orders) && $data['new_user_price']>0){
                $new_user_price+=($data["marketprice"]-$data["new_user_price"])* $goodstotal;//新手会员差价
            }//---end
			$merch_array[$merchid]["ggprice"] += $data["ggprice"];
        ...
        $totalprice=$totalprice-$new_user_price;//新手会员实付款价格
		$grprice = $totalprice;
		...
		$order = array( );
        $order["new_user_price"] = $new_user_price;
		...
		$order['idcard_front'] = $_GPC['idcard_front'];
		$order['idcard_back'] = $_GPC['idcard_back'];
		$order['idnumber'] = $_GPC['idnumber'] . ' ';
		pdo_update('ewei_shop_member', array('idnumber' => $_GPC['idnumber'], 'idcard_front' => $_GPC['idcard_front'], 'idcard_back' => $_GPC['idcard_back']), array('openid' => $_W['openid'], 'uniacid' => $_W['uniacid']));
		//20191029
		foreach ($_GPC['goods'] as $id){
            $gid[]=$id['goodsid'];
        }
        $order['ceyhshopid']=json_encode($gid);//---end
        ...
        if (!empty($ceyhurl) && !empty($item)){
            ...
            if ($ceyhprice>=$item['enoughmoney']){
                $order['price']= bcsub($order['price'],floatval($item['enoughdeduct']),2);
            }//--end
        }
		...
		if( $multiple_order == 0 ) 
		{
			...
			foreach( $allgoods as $goods ) 
			{
				...
				if($goods["goodsbusinesstype"] == 3){//判断是否是海外商品
					$rateprice = round(floatval($order_goods["realprice"]) / (100 + floatval($goods["rate5"])) * 100, 2);
					$rate_price += (floatval($order_goods["realprice"]) - $rateprice);
					$goodsprice1 += floatval($rateprice);
				}else{//非海外商品
					$goodsprice1 += floatval($goods["realprice"]);
				}
              	$goods["rateprice"] = $rate_price;
			}
			if($goodsprice>0){
				pdo_update('ewei_shop_order', array( 'goods_price' => $goodsprice1, 'rateprice' => $rate_price), array( 'id' => $orderid));
			}
		}
		else 
		{
			...
			if($goodsprice>0){
				pdo_update('ewei_shop_order', array( 'goods_price' => $goodsprice1, 'rateprice' => $rate_price), array( 'id' => $og_array[$goodsid]));
			}
		}
		if( $data["type"] == 3 ) 
		{
