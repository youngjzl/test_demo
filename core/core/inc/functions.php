...
//		["pluginname":"PluginModel":private]=> string(7) "seckill" ["set":"PluginModel":private]=> array(0) { }
//		getTaskSeckillInfo（）
//     ewei_shopv2->plugin->seckill（文件夹）->PluginModel(文件)->getTaskSeckillInfo（）(方法)-----秒杀
...
if (!function_exists('mobileUrl')) {
	function mobileUrl($do = '', $query = NULL, $full = false)
	{
		...
		if (empty($query['merchid'])) {
			$merchid = empty(intval($_GPC['merchid'])) ? intval($_W['merchid']) : intval($_GPC['merchid']);
