...
class Category_EweiShopV2Page extends WebPage 
{
	...
	protected function post() 
	{
		...
		$rate1 = floatval(trim($_GPC["rate1"]));
		$rate2 = floatval(trim($_GPC["rate2"]));
		$rate3 = floatval(trim($_GPC["rate3"]));
		$rate4 = floatval(trim($_GPC["rate4"]));
		$rate5 = floatval(trim($_GPC["rate5"]));
		if( !empty($id) ) 
		{
		...
		if( !empty($parentid) ) 
		{
			$parent = pdo_fetch("SELECT id, parentid, name, rate1, rate2, rate3, rate4 ,rate5 FROM " . tablename("ewei_shop_category") . " WHERE id = '" . $parentid . "' limit 1");
			if( empty($parent) ) 
			{
			...
			{
				$parent1 = pdo_fetch("SELECT id, name, rate1, rate2, rate3, rate4 ,rate5 FROM " . tablename("ewei_shop_category") . " WHERE id = '" . $parent["parentid"] . "' limit 1");
			}
		}
		...
		if(empty($rate)) {
			if(!empty($parent)){
				$rate1 = floatval(trim($parent["rate1"]));
				$rate2 = floatval(trim($parent["rate2"]));
				$rate3 = floatval(trim($parent["rate3"]));
				$rate4 = floatval(trim($parent["rate4"]));
				$rate5 = floatval(trim($parent["rate5"]));
			}else{
				$trade = m("common")->getSysset("trade");
				$rate1 = floatval(trim($trade["rate1"]));
				$rate2 = floatval(trim($trade["rate2"]));
				$rate3 = floatval(trim($trade["rate3"]));
				$rate4 = floatval(trim($trade["rate4"]));
				$rate5 = floatval(trim($trade["rate5"]));
			}
		}
		
		//批量修改下级分类，使用上级税率的分类
		pdo_update("ewei_shop_category", array( "rate1" => $rate1, "rate2" => $rate2, "rate3" => $rate3 , "rate4" => $rate4 , "rate5" => $rate5 ), array( "parentid" => $id,"rate"=> 0 ));
		//查询下级类别
		$category = pdo_fetchall("SELECT * FROM " . tablename("ewei_shop_category") . " WHERE uniacid = '" . $_W["uniacid"] . "' and parentid=".$id." ORDER BY parentid ASC, displayorder DESC");
		foreach( $category as $index => $row ) 
		{
			pdo_update("ewei_shop_category", array( "rate1" => $rate1, "rate2" => $rate2, "rate3" => $rate3 , "rate4" => $rate4 , "rate5" => $rate5 ), array( "parentid" => $row["id"],"rate"=> 0 ));
		}
		/*20190418===*/
