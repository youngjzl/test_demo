...
if ($_W['ispost'])
{
    ...
    $rate3 = trim($_GPC["rate3"]);
    $rate4 = trim($_GPC["rate4"]);
    $rate5 = trim($_GPC["rate5"]);
    ...
    //isdiscount_time_end增加这个字段查询
    $sqlgoods = 'SELECT id,title,thumb,marketprice,productprice,minprice,maxprice,isdiscount,isdiscount_time,isdiscount_time_end,isdiscount_discounts,sales,total,description,merchsale FROM ' . tablename('ewei_shop_goods') . ' where id=:id and uniacid=:uniacid limit 1';
    $goodsinfo = pdo_fetch($sqlgoods, array(':id' => $id, ':uniacid' => $_W['uniacid']));
    $goodsinfo = m('goods')->getOneMinPrice($goodsinfo);
//  可以方便查看数据（促销 url:web\sitet.txt;）
// file_put_contents('sitet.txt',var_export($goodsinfo,true).PHP_EOL,FILE_APPEND);
...
if (!(empty($id)))
{
    ...
    if (0 < count($options))
    {
        ...
        if ($canedit)
        {
            ...
            $html .= '<th class="type-4"><div class=""><div style="padding-bottom:10px;text-align:center;">会员价</div><div class="input-group"><input type="text" class="form-control  input-sm option_vip_price_all"  VALUE=""/><span class="input-group-addon"><a href="javascript:;" class="fa fa-angle-double-down" title="批量设置" onclick="setCol(\'option_vip_price\');"></a></span></div></div></th>';
        ...
        while ($i < $newlen)
        {
            ...
            $ids = implode('_', $ids);
            $val = array('id' => '', 'title' => '', 'stock' => '', 'presell' => '', 'costprice' => '', 'productprice' => '','vip_price'=>'', 'marketprice' => '', 'weight' => '', 'virtual' => '');
            ...
            foreach ($options as $o )
            {
                while ($ids === $o['specs'])
                {
                    $val = array('id' => $o['id'], 'title' => $o['title'], 'stock' => $o['stock'], 'costprice' => $o['costprice'], 'productprice' => $o['productprice'], 'presell' => $o['presellprice'], 'vip_price'=> $o['vip_price'],'marketprice' => $o['marketprice'], 'goodssn' => $o['goodssn'], 'productsn' => $o['productsn'], 'weight' => $o['weight'], 'virtual' => $o['virtual']);
            ...
            if ($canedit)
            {
                ...
                //会员价
                $hh .= '<td class="type-4"><input data-name="option_vip_price_' . $ids . '" type="text" class="form-control option_vip_price option_vip_price_' . $ids . '" " value="' . $val['vip_price'] . '"/></td>';
