...
class Index_EweiShopV2Page extends WebPage
{
	...
	protected function selectOrderPrice($day = 0, $is_all = false, $is_avg = false)
	{
		...
		//查询充值记录表2019-8-14
		$sqlcredit2='select id,money,openid,createtime from ' . tablename('ewei_shop_member_log') . ' where status=1 and createtime between :createtime1 and :createtime2';
		$credit2=pdo_fetchall($sqlcredit2, array(':createtime1' => $createtime1, ':createtime2' =>$createtime2));
		...
		//充值余额记录添加到$pdo_res数组2019-8-14
		foreach ($credit2 as $credits2){
			$price += $credits2['money'];
			$member[] = $credits2['openid'];
			$credits2['price']=$credits2['money'];
			unset($credits2['money']);
			array_push($pdo_res,$credits2);
		}
	...
	public function ajaxorder()
	{
		...
		//判断是否是负数代表后台扣掉的余额2019-8-14
		$num_conunt='';
		foreach ($order['fetchall'] as $conunt){
			if ($conunt['price'][0]=='-'){
				$num_conunt+=1;//有几单
			}
		}
		...
		unset($allorder['fetchall']);
		$orders = array('order_count' => $order['count']-$num_conunt, 'order_price' => number_format($order['price'], 2), 'allorder_count' => $allorder['count'], 'allorder_price' => number_format($allorder['price'], 2), 'avg' => number_format($avg['avg'], 2));
