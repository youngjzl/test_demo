...
//			$timeCondition .= " AND o." . $searchtime . "time >= " . $starttime . " AND o." . $searchtime . "time <= " . $endtime . " ";
		...
		if( $_GPC["paytype"] != "" ) 
		{
			if( $_GPC["paytype"] == "2" ) 
			{
				$condition .= " AND ( o.paytype =21 or o.paytype=22 or o.paytype=23 or o.paytype=24 )";
				$priceCondition .= " AND ( o.paytype =21 or o.paytype=22 or o.paytype=23 or o.paytype=24 )";
			}
			else 
			{
				if( $_GPC["paytype"] == "4" ) 
		...
		$paytype = array( array( "css" => "default", "name" => "未支付" ), array( "css" => "danger", "name" => "余额支付" ), 11 => array( "css" => "default", "name" => "后台付款" ), 2 => array( "css" => "danger", "name" => "在线支付" ), 21 => array( "css" => "success", "name" => "微信支付" ), 22 => array( "css" => "warning", "name" => "支付宝支付" ), 23 => array( "css" => "warning", "name" => "银联支付" ), 24 => array( "css" => "warning", "name" => "合利宝支付" ), 3 => array( "css" => "primary", "name" => "货到付款" ), 4 => array( "css" => "primary", "name" => "收银台现金收款" ) );
		...
		if( !empty($list) ) 
		{
			...
			foreach( $list as &$value ) 
			{
				...
				// $value['goodsbussinesstype'] = 'test111';
				...
				foreach( $order_goods as &$og ) 
				{
					...
					/*税费计算*/
					if($og["goodsbusinesstype"] = 3){//判断是否是海外商品
						$rateprice = round(floatval($og["realprice"]) / (100 + floatval($og["rate5"])) * 100, 2);
						$rate_price += (floatval($og["realprice"]) - $rateprice);
						$goodsprice += floatval($rateprice);
					}else{//非海外商品
						$goodsprice += floatval($og["realprice"]);
					}
				}
				unset($og);
				/*税费和商品小计*/
				if($value["goods_price"]==0){
					$value["goods_price"] = $goodsprice;
					$value["rateprice"] = $rate_price;
					
					pdo_update('ewei_shop_order', array( 'goods_price' => $goodsprice, 'rateprice' => $rate_price), array( 'id' => $value["id"]));
				}
				if( !empty($level) && empty($agentid) ) 
				{
		...
		if( $_GPC["export"] == 2 ) 
		{
			...
			$columns = array( 
              ...
              array( "title" => "商品业务类型", "field" => "goodsbusinesstype", "width" => 36 ),
              array( "title" => "身份证号码", "field" => "idnumber", "width" => 36 ),
              array( "title" => "身份证正面", "field" => "idcard_front", "width" => 36 ),
              array( "title" => "身份证反面", "field" => "idcard_back", "width" => 36 )
            );
			if( !empty($agentid) && 0 < $level ) 
			{
			...
			foreach( $list as &$r ) 
			{
				...
				foreach( $ogoods as $k => $g ) 
				{
					...
					// $r["goodsbusinesstype"] = $g["goodsbusinesstype"];
					switch($g['goodsbusinesstype']) {
						case '1':
							$r["goodsbusinesstype"] = '普通商品';
							break;
						case '2':
							$r["goodsbusinesstype"] = '保税商品';
							break;
						case '3':
							$r["goodsbusinesstype"] = '海外直邮';
							break;
						case '4':
							$r["goodsbusinesstype"] = '完税商品';
							break;
						default:
							$r["goodsbusinesstype"] = '';
					}
		...
		if( $_GPC["export"] == 1 ) 
		{
			...
			$columns = array( 
              ...
              array( "title" => "报送时间", "field" => "kong", "width" => 24 ), 
              array( "title" => "电商企业名称", "field" => "merchname", "width" => 24 ), 
              array( "title" => "商品价格", "field" => "goodsprice", "width" => 12 ), 
              array( "title" => "运杂费", "field" => "dispatchprice", "width" => 12 ), 
              array( "title" => "非现金抵扣金额", "field" => "deductprice", "width" => 12 ), 
              array( "title" => "代扣税款", "field" => "rateprice", "width" => 12 ), 
              array( "title" => "实际支付金额", "field" => "price", "width" => 12 ), 
              array( "title" => "订购人注册号", "field" => "uid", "width" => 12 ), 
              array( "title" => "订购人姓名", "field" => "realname", "width" => 12 ), 
              array( "title" => "订购人电话", "field" => "mobile", "width" => 12 ), 
              array( "title" => "订购人证件类型", "field" => "idcard_type", "width" => 12 ), 
              array( "title" => "订购人证件号码", "field" => "idnumber", "width" => 12 ), 
              array( "title" => "支付企业名称", "field" => "paytype", "width" => 24 ), 
              array( "title" => "支付交易编号", "field" => "transid", "width" => 12 ), 
              array( "title" => "收货人姓名", "field" => "realname", "width" => 12 ), 
              array( "title" => "收货人电话", "field" => "mobile", "width" => 12 ), 
              array( "title" => "收货人证件类型", "field" => "idcard_type", "width" => 12 ), 
              array( "title" => "收货人证件号码", "field" => "idnumber", "width" => 12 ), 
              array( "title" => "收货地址", "field" => "address", "width" => 12 ), 
              array( "title" => "收货地址行政区划代码", "field" => "address_send", "width" => 12 ), 
              array( "title" => "发货人", "field" => "kong", "width" => 12 ), 
              array( "title" => "发货人电话", "field" => "kong", "width" => 12 ), 
              array( "title" => "发货人地址", "field" => "kong", "width" => 12 ), 
              array( "title" => "发货人所在国", "field" => "kong", "width" => 24 ), 
              array( "title" => "贸易国别", "field" => "kong", "width" => 24 ), 
              array( "title" => "备注", "field" => "remark", "width" => 12 ), 
              array( "title" => "商品序号", "field" => "goods_id", "width" => 12 ), 
              array( "title" => "企业商品货号", "field" => "goods_goodssn", "width" => 12 ), 
              array( "title" => "企业商品名称", "field" => "goods_title", "width" => 12 ), 
              array( "title" => "企业商品规格型号", "field" => "goods_optiontitle", "width" => 12 ), 
              array( "title" => "企业商品描述", "field" => "goods_subtitle", "width" => 12 ), 
              array( "title" => "条形码", "field" => "goods_productsn", "width" => 12 ), 
              array( "title" => "单位", "field" => "goods_unit", "width" => 12 ), 
              array( "title" => "数量", "field" => "goods_total", "width" => 12 ), 
              array( "title" => "单价", "field" => "goods_price", "width" => 12 ), 
              array( "title" => "总价", "field" => "goods_rprice", "width" => 20 ),
              array( "title" => "原产国", "field" => "kong", "width" => 24 ),
              array( "title" => "商品国检备案号", "field" => "kong", "width" => 24 ),
              array( "title" => "包装种类代码（国检）", "field" => "kong", "width" => 24 ),
              array( "title" => "商品备注", "field" => "kong", "width" => 24 )
            );
			foreach( $list as &$row ) 
			{
				$row["kong"] = " ";
				$row["ordersn"] = $row["ordersn"] . " ";
				$row["goodsprice"] = floatval($row["goodsprice"]) - floatval($row["rate_price"]);
				$row["realname"] = str_replace("=", "", $row["realname"]);
				
				$row["merchname"] = "厦门优铼电子商务有限公司";
				$row["idcard_type"] = "1";
				$row["deductprice"] = floatval($row["deductprice"]) + floatval($row["deductcredit2"]) + floatval($row["deductenough"]) + floatval($row["couponprice"]);
				if (empty($row['addressid'])) 
				{
					$user = unserialize($row['carrier']);
					if (($row['storeid'] != 0) && ($row['ismerch'] == 0)) 
					{
						$user['storename'] = pdo_fetch('SELECT storename FROM ' . tablename('ewei_shop_store') . ' WHERE id = :id and uniacid=:uniacid and status=1', array(':id' => $row['storeid'], ':uniacid' => $_W['uniacid']));
					}
				}
				else 
				{
					$user = iunserializer($row['address']);
					if (!(is_array($user))) 
					{
						$user = pdo_fetch('SELECT * FROM ' . tablename('ewei_shop_member_address') . ' WHERE id = :id and uniacid=:uniacid', array(':id' => $row['addressid'], ':uniacid' => $_W['uniacid']));
					}
					$address_info = $user['address'];
					$row['address'] = $user['province'] . ' ' . $user['city'] . ' ' . $user['area'] . ' ' . $user['street'] . ' ' . $user['address'];
				}
			...
			foreach( $list as &$r ) 
			{
				...
				foreach( $ogoods as $k => $g ) 
				{
					...
					$r["goods_price"] = floatval($g["goods_price"]) - floatval($g["rate_price"]);
		...
		if( $condition != " o.uniacid = :uniacid and o.ismr=0 and o.deleted=0 and o.isparent=0" || !empty($sqlcondition) ) 
		{
			if( $searchfield == "member" ) 
			{
				...
				$openidArr = pdo_fetchall("SELECT openid FROM " . tablename("ewei_shop_member") . " o WHERE uniacid = " . $_W["uniacid"] . $priceCondition);
				if( !empty($openidArr) ) 
				{
