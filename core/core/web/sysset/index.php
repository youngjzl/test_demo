...
class Index_EweiShopV2Page extends WebPage 
{
	...
	public function trade() 
	{
		...
		/*20190418===*/
			//默认税率设置
			$rate1 = floatval(trim($data["rate1"]));
			$rate2 = floatval(trim($data["rate2"]));
			$rate3 = floatval(trim($data["rate3"]));
			$rate4 = floatval(trim($data["rate4"]));
			$rate5 = floatval(trim($data["rate5"]));
			$data["rate1"] = $rate1;
			$data["rate2"] = $rate2;
			$data["rate3"] = $rate3;
			$data["rate4"] = $rate4;
			...
			//查询一级类别
			pdo_update("ewei_shop_category", array( "rate1" => $rate1, "rate2" => $rate2, "rate3" => $rate3 , "rate4" => $rate4 , "rate5" => $rate5 ), array( "parentid" => 0,"rate"=> 0 ));
			$category1 = pdo_fetchall("SELECT * FROM " . tablename("ewei_shop_category") . " WHERE uniacid = '" . $_W["uniacid"] . "' and parentid=0 and rate=0 ORDER BY parentid ASC, displayorder DESC");
			foreach( $category1 as $index1 => $row1 ) 
			{
				pdo_update("ewei_shop_goods", array( "rate1" => $rate1, "rate2" => $rate2, "rate3" => $rate3 , "rate4" => $rate4 , "rate5" => $rate5 ), array( "pcate" => $row1["id"],"rate"=> 0 ));
				
				pdo_update("ewei_shop_category", array( "rate1" => $rate1, "rate2" => $rate2, "rate3" => $rate3 , "rate4" => $rate4 , "rate5" => $rate5 ), array( "parentid" => $row1["id"],"rate"=> 0 ));
				//查询二级类别
				$category2 = pdo_fetchall("SELECT * FROM " . tablename("ewei_shop_category") . " WHERE uniacid = '" . $_W["uniacid"] . "' and parentid=".$row1["id"]." and rate=0 ORDER BY parentid ASC, displayorder DESC");
				foreach( $category2 as $index2 => $row2 ) 
				{
					pdo_update("ewei_shop_goods", array( "rate1" => $rate1, "rate2" => $rate2, "rate3" => $rate3 , "rate4" => $rate4 , "rate5" => $rate5 ), array( "tcate" => $row2["id"],"rate"=> 0 ));
					
					pdo_update("ewei_shop_category", array( "rate1" => $rate1, "rate2" => $rate2, "rate3" => $rate3 , "rate4" => $rate4 , "rate5" => $rate5 ), array( "parentid" => $row2["id"],"rate"=> 0 ));
					$category3 = pdo_fetchall("SELECT * FROM " . tablename("ewei_shop_category") . " WHERE uniacid = '" . $_W["uniacid"] . "' and parentid=".$row2["id"]." and rate=0 ORDER BY parentid ASC, displayorder DESC");
					foreach( $category3 as $index3 => $row3 ) 
					{
						pdo_update("ewei_shop_goods", array( "rate1" => $rate1, "rate2" => $rate2, "rate3" => $rate3 , "rate4" => $rate4 , "rate5" => $rate5 ), array( "tcate" => $row3["id"],"rate"=> 0 ));
					}
				}
			}
		/*20190418===*/
	...
	public function payset() 
	{
		...
		if( $_W["ispost"] ) 
		{
			...
			//添加合利宝支付
			$sec["helipay"]["appid"] = trim($_GPC["data"]["helipay_appid"]);
			$sec["helipay"]["merchid"] = trim($_GPC["data"]["helipay_merchid"]);
			...
			$data = array( );
			//添加合利宝支付
			$data["helipay"] = intval($inputdata["helipay"]);
