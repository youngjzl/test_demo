...
class Goods_EweiShopV2Model 
{
	...
	public function getList($args = array( )) 
	{
		...
		if( !empty($isnew) ) 
		{
		    //20191015追加一个字段条件and isnew_time>=".TIMESTAMP
			$condition .= " and isnew=1 and isnew_time>=".TIMESTAMP;
		}
		...
		if( !$random ) 
		{
			$sql = "SELECT id,title,subtitle,thumb,thumb_url,new_user_price" . $officsql . ",marketprice,productprice,minprice,maxprice,isdiscount,isdiscount_time,isdiscount_discounts,sales,salesreal,total,description,bargain,`type`,ispresell,`virtual`,hasoption,video,bargain,hascommission,nocommission,commission,commission1_rate,commission1_pay,presellprice\r\n            FROM " . tablename("ewei_shop_goods") . " where  " . $condition . " ORDER BY " . $order . " " . $orderby . " LIMIT " . ($page - 1) * $pagesize . "," . $pagesize;
			$total = pdo_fetchcolumn("select count(*) from " . tablename("ewei_shop_goods") . " where  " . $condition . " ", $params);
		}
		else 
		{
			$sql = "SELECT id,title,thumb,thumb_url,new_user_price" . $officsql . ",marketprice,productprice,minprice,maxprice,isdiscount,isdiscount_time,isdiscount_discounts,sales,salesreal,total,description,bargain,`type`,ispresell,`virtual`,hasoption,bargain,hascommission,nocommission,commission,commission1_rate,commission1_pay,presellprice\r\n            FROM " . tablename("ewei_shop_goods") . " where  " . $condition . " ORDER BY rand() LIMIT " . $pagesize;
			$total = $pagesize;
		}
	...
	public function getAllMinPrice($goods) 
	{
		...
		if( is_array($goods) ) 
		{
			...
			$goods[0]["time_dq"]=time();//当前时间
			foreach( $goods as &$value )
			{
				$minprice = $value["minprice"];
				$maxprice = $value["maxprice"];
                //增加一个当前时间用于判断促销
				if( $value["isdiscount"] && $value["time_dq"] >= $value["isdiscount_time"] && $value["time_dq"] <= $value["isdiscount_time_end"])
				{
