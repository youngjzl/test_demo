...
class Finance_EweiShopV2Model 
{
	...
	public function helipay($orderNo = "",$money = 0, $desc = ""){
		require_once("SignService.php");
		$SignService = SignService::instance();
		$sec = m("common")->getSec();
		$sec = iunserializer($sec["sec"]);
		
		$post_data = array( );
		$post_data["productCode"] = "WXPAYOA";
		$post_data["orderNo"] = $orderNo;
		$post_data["merchantNo"] = $sec["helipay"]["merchid"];
		$post_data["refundOrderNo"] = time(). random(4, true);
		$post_data["refundAmount"] = $money;
		$post_data["refundDesc"] = (empty($desc) ? "现金提现" : $desc);
		...
		//请求的url地址
		$url = 'https://cbptrx.helipay.com/cbtrx/rest/refund/refundApply';
		...
		if( $postReturnre_result["errorCode"] == "0000"||$postReturnre_result["errorCode"] == "1130") 
		{
			return true;
		}
		$error = $postReturnre_result["errorCode"] . " | " . $postReturnre_result["errorMessage"] . " | " .$money;
		return error(-2, $error);
	}
}