define(['core', 'tpl', 'biz/plugin/diyform', 'biz/order/invoice'], function(core, tpl, diyform, invoice) {
        ...
        if (number.length > 0) {
            ...
            number.numbers({
                ...
                callback: function(num) {
                    ...
                    var rateprice = core.getNumber(number.closest('.goods-item').find('.rateprice').html()) * num;
                    $('.goodsprice').html(core.number_format(marketprice, 2));
                    $('.rate_price').html(core.number_format(rateprice, 2));
                    modal.caculate()
                }
            })
        }
    ...
    modal.caculate = function() {
        ...
        var showpackageprice2=core.getNumber($(".packageprice2").val());

        var totalprice = goodsprice - taskdiscountprice - lotterydiscountprice - discountprice - isdiscountprice - taskcut-showpackageprice2;
        if ($('.shownum').length > 0) {
    ...
    modal.totalPrice = function(couponprice) {
        ...
        var ceyhmeyhprice=core.getNumber($(".ceyhmeyhprice").html());
        ...
        var totalprice = goodsprice - taskdiscountprice - lotterydiscountprice - discountprice - isdiscountprice - couponprice - buyagainprice-ceyhmeyhprice;
        var taskcut = core.getNumber($("#taskcut").val());
        ...
        var showpackageprice2=core.getNumber($(".packageprice2").html());
        ...
        totalprice = totalprice - merch_enoughprice - enoughprice + dispatchprice - taskcut-showpackageprice2;
        var deductprice = 0;
    ...
    modal.submit = function(obj, token) {
        ...
        var isChecked = $('#xieyi_checked').prop('checked');
        var isdingjincheck=$("#dingjin_checked");
        if (modal.params.mustbind) {}
        if ($this.attr('stop')) {
            return
        }
        if (!isChecked){
            FoxUI.toast.show('请勾选购买协议复选框');
            return
        }
        if (isdingjincheck.length>0 && !isdingjincheck.prop('checked')){
            FoxUI.toast.show('请勾选预售定金协议复选框');
            return
        }
        ...
        if ($("#idFront").val() == '') {
            FoxUI.toast.show('请上传身份证正面照');
            return;
        }
        if ($("#idBack").val() == '') {
            FoxUI.toast.show('请上传身份证反面照');
            return;
        }
        if ($("#idnumber").val() == '') {
            FoxUI.toast.show('请填写身份证号码');
            return;
        }
        ...
        var data = {
            ...
            'idcard_front': $("#idFront").val(),
            'idcard_back': $("#idBack").val(),
