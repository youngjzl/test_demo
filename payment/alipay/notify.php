			...
			if( !is_error($site) ) 
			{
				...
				if( method_exists($site, $method) ) 
				{
					...
					if( $result ) 
					{
						...
						pdo_update("haiguan_date",  array( "initalResponse" => $this->post, "payTransactionId"=>$this->post["trade_no"], "tradingTime"=>$this->post["time_end"], "update_time" => time()), array("orderNo" => $log["tid"] ));
						exit( "success" );
					}
				}
			}
