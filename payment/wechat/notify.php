			...
			if( !is_error($site) ) 
			{
				...
				if( method_exists($site, $method) ) 
				{
					...
					if( $result ) 
					{
						...
						$haiguan = array();
						$haiguan["initalResponse"] = json_encode($this->get);
						$haiguan["payTransactionId"] = $this->get["transaction_id"];
						$haiguan["totalAmount"] = $log["fee"];
						$haiguan["tradingTime"] = date("YmdHis", time());
						$haiguan["update_time"] = time();
						pdo_update("haiguan_data",$haiguan,array("orderNo" => $log["tid"] ));
					}
