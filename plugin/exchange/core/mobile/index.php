...
class Index_EweiShopV2Page extends PluginMobileLoginPage 
{
	...
	public function coupon() 
	{
		...
		if ($exchange == '1') 
		{
			...
			$m = m('member')->getInfo($_W['openid']);			/*判断同级兑换券是否已经存在了*/			$userResult = pdo_fetch('SELECT * FROM ' . $table2 . ' WHERE id = :id AND uniacid = :uniacid and openid = :openid', array(':uniacid' => $_W['uniacid'], ':id' => $codeResult['groupid'],':openid' => $_W['openid']));			if (!empty($userResult[0])) {				show_json(0, '已经兑换过该优惠券');				$this->message('已经兑换过该优惠券');			}			
			$resp = array();
