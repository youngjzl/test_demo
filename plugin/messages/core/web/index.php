...
class Index_EweiShopV2Page extends PluginWebPage 
{
	...
	public function fetch() 
	{
		...
		foreach( $items as $item ) 
		{
			...
			{
				$resp = $this->sendTplNotice($item["openid"], $item["templateid"], $item["nickname"], $id);
			}
			else 
			{
				if( $item["messagetype"] == 1 ) 
				{
                    file_put_contents('sitet.txt',var_export($items,true).PHP_EOL,FILE_APPEND);
	...
	public function sendTplNotice($openid, $templateid, $nickname, $id) 
	{
		global $_W;
		...
		if( !empty($templateid) ) 
		{
			...
			$i++ ) 
			{
				...
				if( stripos($data[$i]["value"], "[推送标题]") !== false ) 
				{
					$data[$i]["value"] = str_replace("[推送标题]", $mess_task["resptitle"], $data[$i]["value"]);
				}
				if( stripos($data[$i]["value"], "[推送描述]") !== false ) 
				{
					$data[$i]["value"] = str_replace("[推送描述]", $mess_task["respdesc"], $data[$i]["value"]);
				}
