									...
									if( !empty($goods) && is_array($goods) ) 
									{
										...
										foreach( $goodsids as $goodsid ) 
										{
											foreach( $goods as $index => $good ) 
											{
												if( $good["id"] == $goodsid ) 
												{
													$childid = rand(10000000, 99999999);
													$childid = "C" . $childid;
						...
						if( $item["params"]["goodsdata"] == "0" ) 
						{
							if( !empty($item["data"]) && is_array($item["data"]) ) 
							{
								...
								if( !empty($goodsids) && is_array($goodsids) ) 
								{
									...
									{
										$goods = pdo_fetchall("select id, showlevels,hascommission,nocommission,commission,commission1_rate,marketprice,commission1_pay,maxprice,showgroups,new_userprice from " . tablename("ewei_shop_goods") . " where id in( " . $newgoodsids . " ) and status=1 and deleted=0 and checked=0 and uniacid=:uniacid order by displayorder desc ", array( ":uniacid" => $_W["uniacid"] ));
									}
									if( !empty($goods) && is_array($goods) ) 
									{
							...
							else 
							{
								...
								else 
								{
									if( 2 < $item["params"]["goodsdata"] ) 
									{
                                        ...
                                        //热卖产品变成后台统计商品
                                        $data_goods_id=array();
                                        if ($item["params"]["goodsdata"] == 11){
                                            $data_goods = pdo_fetchall('SELECT g.id,(select ifnull(sum(og.price),0) from `ims_ewei_shop_order_goods` og left join `ims_ewei_shop_order` o on og.orderid=o.id where o.status>=1 and og.goodsid=g.id and og.uniacid=4) as money from `ims_ewei_shop_goods` g where 1 and g.uniacid=4 order by money desc LIMIT 20');
                                            foreach ($data_goods as $data_good){
                                                $data_goods_id[]=$data_good['id'];
                                            }
                                            $goodsids=implode(',',$data_goods_id);
                                            $goods = pdo_fetchall("select id, title, subtitle, thumb, `type`, minprice, sales, salesreal, total, showlevels, showgroups,hascommission,nocommission,commission,commission1_rate,marketprice,commission1_pay,maxprice,bargain,productprice,ispresell,presellprice,new_user_price from " . tablename("ewei_shop_goods") . " where id in( " . $goodsids . " ) and status=1 and `deleted`=0 and `status`=1 and uniacid=:uniacid ORDER BY FIELD(`id`, $goodsids) limit 20", array( ":uniacid" => $_W["uniacid"] ));
                                        }//-----热卖产品变成后台统计商品 end
										$item["data"] = array( );
										if( !empty($goods) && is_array($goods) ) 
										{
																				...
																				else 
																				{
																					if( $item["id"] == "tabbar" ) 
																					{
																						if( !empty($item["data"]) ) 
																						{
																							...
																							$activeIndex = 0;
//                                                                                            $item["data"]['C1570852450142']=
//                                                                                           var_dump($item["data"]);
																							foreach( $item["data"] as $childid => $child ) 
																							{
																								$active = false;
																								if ($childid=='C1570852450142'){
                                                                                                    $item["data"][$childid]["linkurl"]='';
                                                                                                }
