...
//		}
		...
		if (trim($_GPC['creditshop']) == 1) {
			...
			$query_field = 'id,cates,displayorder,title,unit,subtitle,keywords,type,goodsbusinesstype,isnew,issendfree,isnodiscount,marketprice,productprice,total,sales,cashier,cash,quality,seven,invoice,repair,status,hasoption,thumb,thumb_url,content,minprice,bargain';
		...
		if (!empty($list)) {
			foreach ($list as &$li) {
				...
				$li['thumb_url'] = array_merge(array($li['thumb']), iunserializer($li['thumb_url']));
				//2019-9-2增加商品多张图片
				if (!empty($li['thumb_url'][0])){
					$li['thumb_url'][0]=substr($li['thumb_url'][0],24);
				}
			}
		}
		$categorys = m('shop')->getFullCategory(true);
		//cates分类2019-8-30
		foreach ($list as $key=>$list_cates){
			$list_cates['cates']=explode(',',$list_cates['cates']);
			foreach ($list_cates['cates'] as $k=>$list_catesid){
				foreach ($categorys as $cate){
					if ($cate['id']==$list_catesid){
						$list_cates['cates'][$list_catesid]=$cate['name'];
						unset($list_cates['cates'][$k]);
					}
				}
			}
			$list[$key]['cates']=$list_cates['cates'];
		}
		//--end
		...
		if (empty($list) && $page !== 1) {
			...
			include $this->template('utiltwo/goods_selector');
		}
	}
	...
	public function op()
	{
		...
		include $this->template('utiltwo/goods_selector_op');
	}

	public function js()
	{
		global $_W;
		include $this->template('utiltwo/goods_selector_js');
	}
